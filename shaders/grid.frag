...
vec4 grid(vec3 fragPos3D, float scale)
{
    ...
    bool blue = fragPos3D.x > -0.1 * minimumx && fragPos3D.x < 0.1 * minimumx;
    bool red  = fragPos3D.z > -0.1 * minimumz && fragPos3D.z < 0.1 * minimumz;

    color.b += float(blue) * 0.8;
    color.r += float(red)  * 0.8;
